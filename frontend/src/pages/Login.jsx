...
function Login() {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(false)
    if (res.ok) {
      setMessage({ text: 'Account created! Sign in now.', type: 'success' })
      setView('main')
      setTab('signin')
    } else {
      setMessage({ text: data.message, type: 'error' })
        ...
        {view === 'otp' ? (
          <>
            ...
            <p className={styles.switch}>
              <button className={styles.link} onClick={handleResendOtp} disabled={loading}>
                {loading ? 'Resending...' : 'Resend OTP'}
              </button>
              {' · '}
              <button className={styles.link} onClick={() => { setView('main'); setMessage({ text: '', type: '' }) }}>Wrong email?</button>
            </p>
          </>
        ) : (
          <>
              ...
              {tab === 'signin' ? (
                ...
                >
                  ...
                  <form className={styles.form} onSubmit={handleSendOtp}>
                    <input className={styles.input} type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required />
                    <button className={styles.btn} type="submit" disabled={loading}>
                      {loading ? <><span className={styles.spinner} /> Sending...</> : 'Send OTP'}
                    </button>
                  </form>
                  <div className={styles.divider}><span>or</span></div>
                  <GoogleBtn />
